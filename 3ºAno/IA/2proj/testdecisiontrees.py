...
for file in files:
    ...
    try:
        if file[0] == 'a' or file[0] == 't':
            ...
            print("points", round(points / 26 * 8, 1), "+ reduced tree ", round(pointsshort / 2 * 3, 1), "+ noise",
                  round(pointsgen / 4 * 3, 1))
            C.append([file, round(points / 26 * 8, 1), round(pointsshort / 2 * 3, 1), round(pointsgen / 4 * 3, 1)])
            print("\nExpected grade:\t",
                  round(points / 26 * 8, 1) + round(pointsshort / 2 * 3, 1) + round(pointsgen / 4 * 3, 1),
                  "(/14)+hidden tests (4pt)+report (2pt)")
    except:

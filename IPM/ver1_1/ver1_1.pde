...
void draw()
{
  ...
  Target secondClosest = getTargetBounds(0);
  for (int i = 1; i < 16; i++) 
  {
    Target target = getTargetBounds(1);
    if (dist(firstClosest.x, firstClosest.y, mouseX, mouseY) < dist(target.x, target.y, mouseX, mouseY))
    {
  ...
  float conDi = firstClosest.w + dist(firstClosest.x, firstClosest.y, mouseX, mouseY);
  float intDj = dist(secondClosest.x, secondClosest.y, mouseX, mouseY) - secondClosest.w;
  float cursorRadius = min(conDi, intDj); 
  */
  circle(mouseX, mouseY, PPCM * 1.5);
  //if (cursorRadius == intDj)
  //{

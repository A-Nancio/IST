...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        for i in range(layers + 1):
            ...
            else: input_neurons = hidden_size
            if i == layers:
                output_neurons = n_classes
            else: output_neurons = hidden_size

            aux[i] = nn.Linear(input_neurons, output_neurons)

        self.layers = nn.ModuleList(aux)
...
def main():
    ...
    # plot
    plot(epochs, train_mean_losses, ylabel='Loss', name='training-loss-' + os.path.basename(__file__))
    plot(epochs, valid_accs, ylabel='Accuracy', name='validation-accuracy-' + os.path.basename(__file__))

...
def set_elementsM(ax: plt.Axes = None, title: str = '', xlabel: str = '', ylabel: str = '', percentage: bool = False):
    if ax is None:
        ax = plt.gca()
    ax.set_title(title)
    ax.set_xlabel(xlabel)
...
def multiple_line_chart(xvalues: list, yvalues: dict, ax: plt.Axes = None, title: str = '', xlabel: str = '',
                        ylabel: str = '', percentage: bool = False, rotation: bool = False, M: bool = False):
    if M:
        ax = set_elementsM(ax=ax, title=title, xlabel=xlabel, ylabel=ylabel, percentage=percentage)
    else:
        ax = set_elements(ax=ax, title=title, xlabel=xlabel, ylabel=ylabel, percentage=percentage)
    set_locators(xvalues, ax=ax, rotation=rotation)
    legend: list = []
    for name, y in yvalues.items():
        if M:
            ax.plot(xvalues, y, marker='o', linestyle='--',linewidth=1, markersize=2)
        else:
            ax.plot(xvalues, y)
        legend.append(name)
